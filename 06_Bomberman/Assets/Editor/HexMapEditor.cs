...
public class HexMapEditor : Editor
{
	...
	public override VisualElement CreateInspectorGUI()
	{
		...
		VisualTreeAsset visualTree = AssetDatabase.LoadAssetAtPath<VisualTreeAsset>("Assets/Editor/HexMapInspector.uxml");
		...
		customTileTypeField.RegisterValueChangedCallback((ChangeEvent<Enum> change) => { _customCellType = (CellType)change.newValue; });
	...
	private void SetCustomTilesButton_clicked()
	{
		...
		foreach (CellPresenter presenter in Selection.gameObjects.Select(g => g.GetComponent<CellPresenter>()))
		{
			...
			_selectedPositions.Add(HexCoordinateConverter.ConvertVector3ToCoordinate(presenter.transform.localPosition));
		}
	...
	private void GenerateMap()
	{
		...
		//Spawn tiles
		Dictionary<HexPosition, CellType> customCellTypes = definition.GetCustomCellTypes();
		foreach (HexPosition hexCoordinate in definition.GetAllCoordinates())
		{
			...
			spawnedTile.transform.localPosition = HexCoordinateConverter.ConvertCoordinateToVector3(hexCoordinate);
